        ...
        if in_labels_block:
            if current_indent <= indent_level and not line.lstrip().startswith("-"):
                ...
                if val:
                    ...
                    if not (
                        ":" in val
                        and not val.startswith("type:")
                        and not val.startswith("area:")
                        and not val.startswith("priority:")
                        and not val.startswith("status:")
                        and not val.startswith("release:")
                    ):
    ...
    def check_labels(labels_list, filepath, context):
        ...
        for lbl in labels_list:
            ...
            if lbl not in defined_labels:
                print(
                    f"Error: Unknown label '{lbl}' referenced in {filepath.relative_to(root_dir)} ({context})"
                )
                issues_found = True
    ...
    for wf in workflow_files:
        with open(wf) as f:
            ...
            for match in re.finditer(r"has_label\s+([\w:\-\*]+)", content):
                ...
                if lbl not in defined_labels:
                    print(
                        f"Error: Unknown label '{lbl}' referenced in {wf.relative_to(root_dir)} (has_label script)"
                    )
                    issues_found = True
